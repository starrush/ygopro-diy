...
function c114001278.checkop(e,tp,eg,ep,ev,re,r,rp)
	...
	local p2=false
	while tc do
		if not ( tc:IsSetCard(0x221) or tc:IsSetCard(0xabb) ) then
...
function c114001278.sumlimit(e,c,sump,sumtype,sumpos,targetp,se)
	return not ( c:IsSetCard(0x221) or c:IsSetCard(0xabb) )
end
--
...
function c114001278.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) then
		if Duel.SpecialSummonStep(c,0,tp,tp,false,false,POS_FACEUP) then
			if Duel.GetFieldGroupCount(tp,LOCATION_MZONE,0)==1 and Duel.IsExistingMatchingCard(Card.IsCode,tp,LOCATION_GRAVE,0,1,nil,113999965) then
				local e1=Effect.CreateEffect(c)
